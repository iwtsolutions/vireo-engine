        ...
        setImmediate(function () {
            self.initializeCollections(models, function (error) {
                ...
                Object.keys(models).forEach(function (m) {
                    self.models[m] = {
                        find: getFindFunc.call(self, m),
                        findOne: getFindOneFunc.call(self, m),
                        save: getSaveFunc.call(self, m)
                    };
                });
                self.emit('ready');
            });
...
function getSaveFunc(model) {
    let save = this.getSaveFunc(model);
    return function (data, callback) {
        ...
        // TODO: Validate required data from model
        save(result, callback);
    };
}

function getFindFunc(model) {
    let find = this.getFindFunc(model);
    let save = getSaveFunc.call(this, model);
...
function getFindOneFunc(model) {
    let findOne = this.getFindOneFunc(model);
    let save = getSaveFunc.call(this, model);
